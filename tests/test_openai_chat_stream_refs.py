...
class OpenAIChatGenerateFallbackTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_edit_uses_images_api_fallback_before_file_service(self):
        ...
        backend = mod.OpenAIChatImageBackend(
            ...
            base_url="https://api.example.com/v1",
            api_keys=["test-key"],
            default_model="grok-imagine-image-edit",
        )
        backend._get_client = lambda key: client
    ...
    async def test_edit_routes_chinese_non_chat_error_to_images_api(self):
        mod = _load_module()
        imgr = _DummyImageManager()
        client = _DummyClient([RuntimeError("模型不支持文本对话")])
        backend = mod.OpenAIChatImageBackend(
            imgr=imgr,
            base_url="https://api.example.com/v1",
            api_keys=["test-key"],
            default_model="gateway-image-model",
        )
        backend._get_client = lambda key: client
