...
class ProviderRegistry:
    ...
    def _resolve_template_key(cls, item: dict) -> str:
        ...
        if "poll_interval" in item or "poll_timeout" in item:
            return "gitee_async"
        if "cookie_list" in item or "apikey" in item:
            return "jimeng"
        if "server_url" in item and "empty_response_retry" in item:
            return "grok_video"
        if "full_generate_url" in item:
            return "openai_full_url_images"
        if "num_inference_steps" in item and "negative_prompt" in item:
            return "gitee_images"
        ...
        if "generativelanguage.googleapis.com" in base_url:
            return "gemini_native"
        # 有 api_url 但没有 base_url 的通常是 flow2api
        if item.get("api_url") and not item.get("base_url"):
            return "flow2api"
        # 默认：凡有 base_url 或 api_keys 的都当作 openai_images
        if item.get("base_url") or item.get("api_keys"):
            return "openai_images"
        return ""
    ...
    def _build_backend(self, pid: str, template_key: str, conf: dict) -> object:
        ...
        if template_key == "grok_images":
            return GrokImagesBackend(
                ...
                default_size=str(conf.get("default_size") or "2048x2048").strip(),
