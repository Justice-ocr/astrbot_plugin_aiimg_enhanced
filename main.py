...
"""
    ...
    async def aiimg_generate(
        ...
        1. mode=edit：用户想修改/编辑某张已有图片时使用。
           触发条件（满足任意一条即可）：
           - 当前消息或引用消息中有图片，且用户要求修改（换颜色、换背景、换风格、换衣服、换细节等）
           - 用户说"把XX换成YY"、"把这张图改成..."、"帮我修一下..."等明显针对某张图的请求
           注意：工具只能获取当前消息或引用消息中的图片。
           如果用户请求改图但消息中没有图片，请先询问用户提供图片。
        ...
        【常见场景示例】：
        - 用户发了图片+说"把白丝换成黑丝" → mode=edit，prompt="把白色丝袜换成黑色"
        - 用户发了图片+说"换个背景" → mode=edit
        - 用户只说"白丝换黑丝"但没发图片 → 先询问用户提供图片，再调用mode=edit
        - "画一只猫" → mode=text
